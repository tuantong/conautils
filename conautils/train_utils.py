...
from conautils.model_utils import set_seeds
from conautils.predictions_utils import combine_forecasts
...
def fit_and_predict_chunks(
    ...
    def process_group(group):
        # Remove leading zeros until the remaining elements are at least 2
        # to avoid single value in the training set
        mask = find_leading_zeros(group[target_col], min_length=2)
    ...
    if num_workers > 1:
        pred_df = _parallel_fit_and_predict(
            id_data_configs_list,
            num_workers,
            log_interval,
            id_col,
...
def _parallel_fit_and_predict(
    id_data_configs_list, num_workers, log_interval, id_col, target_col, date_col, df_events_all, # custom-event
):
    pred_df_list = []
    ...
    return pd.concat(pred_df_list, ignore_index=True)


def _sequential_fit_and_predict(
    id_data_configs_list, log_interval, id_col, target_col, date_col, df_events_all # custom-event
):
    pred_df_list = []
    ...
    tqdm_out = TqdmToLoguru(logger)
    pbar = tqdm(
        total=len(id_data_configs_list),
        desc="Processing IDs",
        file=tqdm_out,
    )
    completed_count = 0
    for id_data_config_dict in id_data_configs_list:
        pred_df = fit_and_predict_single_id(
            id_data_config_dict, id_col, target_col, date_col, df_events_all # custom-event
        )
        pred_df_list.append(pred_df)
        completed_count += 1
    ...
    pbar.update(len(id_data_configs_list) - pbar.n)
    pbar.close()
...
def fit_and_predict_single_id(
    ...
    if df_events_untrain2 is not None and len(df_events_untrain2) > 0: # custom-event
        forecast_combine = evented_forecast(forecast_combine, df_events_untrain2) # custom-event
    os.makedirs(PROJECT_DEBUG_PATH, exist_ok = True) # custom-event
    forecast_combine.to_csv(os.path.join(PROJECT_DEBUG_PATH, f"forecast_combine_{id_}.csv"),index=False) # custom-event
...
def fit_model_with_events(df, model_configs, freq, minimal, progress, use_events, df_events_all): # custom-event
    ...
    if df_events_all is not None and len(df_events_all) > 0: # custom-event
